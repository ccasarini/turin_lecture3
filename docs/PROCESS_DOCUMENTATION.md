...
A high-level review of the speech was performed to identify primary policy pillars. This resulted in `process/summary.md`, which categorizes the address into five key themes:
1. Security & Crime
...
The `process/process_sona.py` script utilizes a chunking strategy to process the text without missing context:
...
Each extracted commitment is mapped to a predefined schema in `process/commitments.csv`:
...
- `/process/process_sona.py`: The Python script used to parse and structure the commitments.
- `/process/commitments.csv`: The final dataset containing 37 distinct commitments.
...
To ensure 100% accuracy, every entry in `process/commitments.csv` includes an `Exact Quotes` field. 
...
- **Automation:** The `process/process_sona.py` script can be re-run to regenerate the CSV if new extraction patterns are added.

## 6. Usage Instructions
1. **To extend the analysis:** Modify the `commitments` list in `process/process_sona.py` to include new patterns or manually identified promises.
2. **To update the dataset:** Run `python3 process_sona.py` from the `process/` directory.
3. **To query the data:** Import `process/commitments.csv` into Excel, Power BI, or a Python Pandas environment for further visualization or policy tracking.
